...
## 7. 文件操作
Python 文件，包括文件类型处理、文件打开、文件位置定位、文件读取、文件写入和文件关闭。
...
### 7.2 open（）函数<br>
函数基本使用语法：文件句柄 = open('文件路径', '模式'，编码方式)<br>
其中：<br>
1、文件路径：主要有两种，一种是使用相对路径，想上面的例子就是使用相对路径,另外一种就是绝对路径，像' C:/Users/shu/Desktop/python/test.txt' <br>
...
f=open('test.txt','r')
data=f.read()
print(data)           #梦入江南烟水路，行尽江南，不与离人遇。
f.close() 

2.只写模式，w  (存在会覆盖原来内容）
f=open('test.txt','w')
f.write('作者：芊芊的小鱼儿')
f.close()              #写完后原来的内容全都不见了，只剩下'作者：芊芊的小鱼儿'.

3、追加模式，a
f=open('test.txt','a')
f.write('作者：芊芊的小鱼儿')
f.close()              #写完内容如下：梦入江南烟水路，行尽江南，不与离人遇。作者：芊芊的小鱼儿

4、以r+模式打开
f=open('test.txt','r+')
f.write('作者：芊芊的小鱼儿')
print(f.read())         # 输出为" 路，行尽江南，不与离人遇。"         
f.close()            
#写完内容为 “ 作者：芊芊的小鱼儿路，行尽江南，不与离人遇。”
#为什么输出和写完内容会这样呢？ 解释这个只要知道紧跟光标的位置就可以明白了。
　　首先，以r方式打开光标都会在最开始的位置，这时候执行f.write('作者：芊芊的小鱼儿') ，这时候就把原来的‘自恐多情损梵行’覆盖了。这时候光标移动到','之前，
紧接着进行print(f.read())。就会把后面剩下的文本内容打印出来。所以输出就是：" 路，行尽江南，不与离人遇。" 整个程序完成后，
整个文本内容就成了；“ 作者：芊芊的小鱼儿路，行尽江南，不与离人遇。”（光标！！）
```

### 7.3 read()函数 : 按 字节 / 字符 读取文件 

文件对象（open() 函数的返回值）提供了 read() 函数可以按字节或字符读取文件内容，到底是读取字节还是字符，取决于使用 open() 函数打开文件时，是否使用了 b 模式，如果使用了 b 模式，则每次读取一个字节；反之，则每次读取一个字符。

```py
file.read([size]) # file 表示打开的文件对象；size 作为一个可选参数，用于指定要读取的字符个数，如果省略，则默认一次性读取所有内容。
